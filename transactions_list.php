<?php
session_start();
require_once 'config.php';

if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    mysqli_query($conn,"DELETE FROM stock_transaction WHERE TransactionID=$id");
    header("Location: transactions_list.php");
    exit;
}

$sql = "
SELECT st.TransactionID, st.timeStamp, st.amount, st.StockID,
       tp.name AS participant, sf.name AS staffName
FROM stock_transaction st
LEFT JOIN transaction_participant tp ON st.ParticipantID = tp.ParticipantID
LEFT JOIN staff sf ON st.StaffID = sf.StaffID
ORDER BY st.timeStamp DESC
";
$res = mysqli_query($conn,$sql);
?>

<!DOCTYPE html>
<html>
<head>
  <title>FIS Admin - Transactions</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <style>
    body {
      background: linear-gradient(135deg, #e1f5fe, #fce4ec);
      min-height: 100vh;
      font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
    }
    .page-wrapper { max-width: 1200px; margin: 30px auto; }
    .page-card {
      border-radius: 14px;
      overflow: hidden;
      box-shadow: 0 15px 35px rgba(0,0,0,0.08);
      border: none;
    }
    .page-card-header {
      background: linear-gradient(90deg, #2196f3, #e91e63);
      color: #fff;
    }
    .table thead {
      background: linear-gradient(90deg,#2196f3,#e91e63);
      color: #fff;
    }
  </style>
</head>
<body>
<div class="page-wrapper">
  <div class="card page-card">
    <div class="card-header d-flex justify-content-between align-items-center page-card-header">
      <div>
        <h4 class="mb-0">Stock Transactions</h4>
        <small class="text-light-50">Executed trades recorded by the system.</small>
      </div>
      <a href="dashboard_system.php" class="btn btn-outline-light btn-sm">Back to Dashboard</a>
    </div>
    <div class="card-body">

      <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="transaction_form.php" class="btn btn-success btn-sm">+ Add Transaction</a>
        <span class="text-muted small">
          Total transactions: <strong><?php echo mysqli_num_rows($res); ?></strong>
        </span>
      </div>

      <div class="table-responsive">
        <table class="table table-sm table-hover align-middle mb-0">
          <thead>
            <tr>
              <th>ID</th>
              <th>Time</th>
              <th>Amount</th>
              <th>Stock</th>
              <th>Participant</th>
              <th>Staff</th>
              <th class="text-end">Actions</th>
            </tr>
          </thead>
          <tbody>
          <?php while($r = mysqli_fetch_assoc($res)): ?>
            <tr>
              <td><?php echo $r['TransactionID']; ?></td>
              <td><?php echo $r['timeStamp']; ?></td>
              <td><?php echo $r['amount']; ?></td>
              <td><?php echo $r['StockID']; ?></td>
              <td><?php echo htmlspecialchars($r['participant']); ?></td>
              <td><?php echo htmlspecialchars($r['staffName']); ?></td>
              <td class="text-end">
                <a href="transaction_form.php?id=<?php echo $r['TransactionID']; ?>" class="btn btn-primary btn-sm">Edit</a>
                <a href="transactions_list.php?delete=<?php echo $r['TransactionID']; ?>"
                   onclick="return confirm('Delete this transaction?');"
                   class="btn btn-danger btn-sm">Delete</a>
              </td>
            </tr>
          <?php endwhile; ?>
          </tbody>
        </table>
      </div>

    </div>
  </div>
</div>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
